...
def writeResultToFile(filename, result):
    ...
    print()
    print('results stored under a .csv file')

dynatraceApi = DynatraceApi(env, apiToken, verifySSL)

# retireve all security problems
securityAttacks = dynatraceApi.getAttacks()

# if the details flag is set, retrieve the details for every security problem
# write result to a CSV file
if showDetails:
    securityAttackDetails = []
    for secP in securityAttacks:
        securityAttackDetail = dynatraceApi.getAttackDetails(secP["attackId"])
        securityAttackDetails.append(securityAttackDetail)
    writeResultToFile('securityAttackDetails.csv', securityAttackDetails)
else:
    writeResultToFile('securityAttacks.csv', securityAttacks)
