...
def writeResultToFile(filename, result):
    ...
    print()
    print('results stored under securityClvProblemDetails.csv')

dynatraceApi = DynatraceApi(env, apiToken, verifySSL)

# retireve all security problems
securityProblems = dynatraceApi.getClvSecurityProblems()
...
if showDetails:
    ...
    for secP in securityProblems:
        securityProblemDetail = dynatraceApi.getClvSecurityProblemDetails(secP["securityProblemId"])
        securityProblemDetails.append(securityProblemDetail)
    writeResultToFile('securityClvProblemDetails.csv', securityProblemDetails)
else:
    writeResultToFile('securityClvProblems.csv', securityProblems)
